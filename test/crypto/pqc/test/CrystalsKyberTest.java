...
//        testConcurrency();
     ...
     public void testKyber()
     {
         ...
         */
         ...
         keyGen.init(new KyberKeyGenerationParameters(random, KyberParameters.kyber512));
...
//         assertArrayEquals(Hex.decode(expectedSharedSecret), decryptedSharedSecret);
     ...
     private void testPlantard() {
        ...
        short[] test_input = new short[256];
        int i;
        for (i = 0; i < KyberEngine.KyberN; i++) {
            test_input[i] = (short) 100;
        }

        short[] ntt_input = Ntt.ntt(test_input);
        short[] ntt_inverted = Ntt.invNtt(ntt_input);

        Helper.printShortArray(ntt_inverted);
    ...
    private void testConcurrency() throws Exception {
        ...
        rt.gc();
        long total_memory = rt.totalMemory() - rt.freeMemory();
        AsymmetricCipherKeyPair keyPair = keyGen.generateKeyPair();
        rt.gc();

        long keypair_memory = total_memory - (rt.totalMemory() - rt.freeMemory());
        System.out.println("Used memory in bytes: " + keypair_memory);
        KyberKEMGenerator kemGen = new KyberKEMGenerator(random);

        SecretWithEncapsulation secretEncap = kemGen.generateEncapsulated(keyPair.getPublic());
        rt.gc();

        long encrypt_memory = total_memory - (rt.totalMemory() - rt.freeMemory());
        System.out.println("Used memory in bytes: " + (encrypt_memory));
        ...
        long decrypt_memory = total_memory - (rt.totalMemory() - rt.freeMemory());

        System.out.println("Used memory in bytes: " + (decrypt_memory));
