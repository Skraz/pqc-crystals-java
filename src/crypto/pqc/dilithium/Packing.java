...
public class Packing
{
    ...
    static void unpackSecretKey(PolyVecK t0, PolyVecL s1, PolyVecK s2, byte[] secretKey, DilithiumEngine engine)
    {
        ...
        {
            s1.getVectorIndex(i).polyEtaUnpack(secretKey, end + i * engine.getDilithiumPolyEtaPackedBytes());
        }
        ...
        for (i = 0; i < engine.getDilithiumK(); ++i)
        {
            s2.getVectorIndex(i).polyEtaUnpack(secretKey, end + i * engine.getDilithiumPolyEtaPackedBytes());
        }
        ...
        for (i = 0; i < engine.getDilithiumK(); ++i)
        {
            t0.getVectorIndex(i).polyt0Unpack(secretKey, end + i * DilithiumEngine.DilithiumPolyT0PackedBytes);
        }
    }

    /**
     * Bit-pack signature sig = (c, z, h).
     * @return Bit packed signature Byte Array
     */
    static byte[] packSignature(byte[] c, PolyVecL z, PolyVecK h, DilithiumEngine engine)
    {
        ...
        // Helper.printByteArray(outBytes);
